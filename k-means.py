...
x = generated_simulated_data()  # 2D array of shape (2, 4000)
N_CLUSTERS = 1
N_SAMPLES = 4000
FEAT_DIM = 2
EPS = 0.000001
...
def not_change_detect(assingment_prev, assignment_new):
    """
    Detect if the assignment has no changed.
    """
    print(np.mean(assingment_prev == assignment_new))    
    return np.mean(assingment_prev == assignment_new)>0.95
...
while iter < MAX_ITER:
    ...
    print(f"New centers from assignment: {c}")
    print(f"iter: {iter}")
    iter += 1
    ...
    if not_change_detect(assignment_prev, assignments):
        ...
        print(f"New centers from split: {c}")
        print(f"iter: {iter}")
    if iter == MAX_ITER:
